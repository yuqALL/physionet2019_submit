...
# 计算3h内变化最大值的众数
def getRangeIn3h(features):
    time = [x[0] for x in features]
    value = [x[1] for x in features]
    beginTime = time[0]
    beginI = 0
    valueRange = []
    for i in range(len(features)):
        #        if time[i] - beginTime < 1 :
        #
        if time[i] - beginTime > 3:
            minValue = np.min(value[beginI:i])
            maxValue = np.max(value[beginI:i])
            valueRange.append(maxValue - minValue)
            t = beginI + 1
            while time[t] < beginTime + 1:
                t = t + 1
            beginI = t
            beginTime = time[t]
    if len(valueRange) == 0:
        valueRange.append(-1)
    rangeMax = np.max(valueRange)
    rangeMin = np.min(valueRange)
    rangeMean = np.mean(valueRange)
    #    rangeVar = np.var(valueRange)
    rangeMode = stats.mode(valueRange)[0][0]
    return rangeMax, rangeMin, rangeMean, valueRange, value
